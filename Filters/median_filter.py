...
def median_filter(pixel, start, filter_size):
    ...
    for i in range(start, pixel.shape[0]-start):
        for j in range(start, pixel.shape[1]-start):
            new_pixel[i][j] = int(np.median(pixel[i-start:i+filter_size-start, j-start:j+filter_size-start]))
...
new_pixel = median_filter(pixel, start, filter_size)
b = 4
